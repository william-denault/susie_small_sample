...
for (n in n_values) {
  for (h in h_values) {
    ...
    writeLines(paste('  if(file.exists("/project2/mstephens/wdenault/susie_small_sample/simulations/cor_small_sample_susie', n, '_h', h, '_unfilter_L1.rds")){\n', sep=""), fileConn)
    writeLines(paste('   temp= readRDS("/project2/mstephens/wdenault/susie_small_sample/simulations/cor_small_sample_susie', n, '_h', h, '_unfilter_L1.rds")\n', sep=""), fileConn)
    ...
    writeLines("    temp <- temp0\n", fileConn)
    writeLines(paste('  }\n', sep=""), fileConn)
    writeLines(paste('  saveRDS(temp, file="/project2/mstephens/wdenault/susie_small_sample/simulations/cor_small_sample_susie', n, '_h', h, '_unfilter_L1.rds")\n', sep=""), fileConn)
    writeLines("}\n", fileConn)
