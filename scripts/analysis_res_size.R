...
load("~/susie_small_sample/simulations/small_sample_susie10_h75.RData")
obs_size <- rep( NA, 10)
for ( i in 1:10){
  obs_size[i] <-  mean( temp[which(temp[,3] == (i )),4] )
}
...
library(ggplot2)
P14 <- ggplot( df, aes(y=obs_size, x=as.factor(L), col=BF))+
  geom_point(
  )+
  ylab(' ')+
  xlab(' ')+
...
library(ggplot2)
P24 <- ggplot( df, aes(y=obs_size, x=as.factor(L), col=BF))+
  geom_point(
  )+
  ylab(' ')+
  xlab(' ')+
